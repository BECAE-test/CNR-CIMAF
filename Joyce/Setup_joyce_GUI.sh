...
# ==================== CONFIG ====================
APP_NAME="${APP_NAME:-joyce}"                         # venv name (final path: ~/.local/share/virtualenvs/joyce)
REQ_FILE="${1:-requirements.txt}"                     # requirements file (default: requirements.txt)
JOYCE_VENVS_DEFAULT="${HOME}/.local/share/virtualenvs"
JOYCE_VENVS="${JOYCE_VENVS:-$JOYCE_VENVS_DEFAULT}"    # override: JOYCE_VENVS=/custom/path ./install.sh
VENV_PATH="${JOYCE_VENVS}/${APP_NAME}"

# name of the variable pointing to bin/activate (default: MY_VENV)
...
for rc in "$BASHRC" "$ZSHRC"; do
  [ -f "$rc" ] || continue
  append_once "$rc" ""
  append_once "$rc" "# --- JOYCE env ---"
  append_once "$rc" "export JOYCE_VENVS=\"$JOYCE_VENVS\""
  append_once "$rc" "export $VENV_ACTIVATE_VAR=\"$VENV_PATH/bin/activate\""
  append_once "$rc" "alias JOYCE='source \"\$$VENV_ACTIVATE_VAR\"'"
  append_once "$rc" "# --- end JOYCE env ---"
done
...
echo "   Please reload your shell configuration (e.g. run: source ~/.bashrc)"
