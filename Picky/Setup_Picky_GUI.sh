...
# ==================== CONFIG ====================
APP_NAME="${APP_NAME:-picky}"                       # venv name (default: picky)
REQ_FILE="${1:-requirements.txt}"                   # requirements file (default: requirements.txt)
PICKY_VENVS_DEFAULT="${HOME}/.local/share/virtualenvs"
PICKY_VENVS="${PICKY_VENVS:-$PICKY_VENVS_DEFAULT}"  # override: PICKY_VENVS=/custom/path ./install_picky_env.sh
VENV_PATH="${PICKY_VENVS}/${APP_NAME}"
...
for rc in "$BASHRC" "$ZSHRC"; do
  [ -f "$rc" ] || continue
  append_once "$rc" ""
  append_once "$rc" "# --- picky env ---"
  append_once "$rc" "export PICKY_VENVS=\"$PICKY_VENVS\""
  append_once "$rc" "export $VENV_ACTIVATE_VAR=\"$VENV_PATH/bin/activate\""
  append_once "$rc" "alias PICKY='source \"\$$VENV_ACTIVATE_VAR\"'"
  append_once "$rc" "# --- end picky env ---"
done
